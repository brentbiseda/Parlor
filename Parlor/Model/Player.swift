...
extension GameKind {
    ...
    var leaderboardTitle: String? {
        switch self {
        case .pinball, .breakout, .tetris, .capsules, .muncher, .hopper, .centipede, .snake, .bomberman, .solarStriker, .trivia:
            return "High scores"
    ...
    func leaderboardLabel(for value: Int) -> String {
        switch self {
        case .pinball, .breakout, .tetris, .capsules, .muncher, .hopper, .centipede, .snake, .bomberman, .solarStriker, .trivia, .football:
            return "\(value) pts"
